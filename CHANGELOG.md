# @embeddable.com/remarkable-ui

## 3.0.23

### Patch Changes

- e99075c: Add box-shadow to card component

## 3.0.22

### Patch Changes

- 4cce365: PivotTable: when multiple column aggregations (Sum, Min, Max, Average) are shown, every total row now stays sticky, stacked at the bottom, instead of only the final one being pinned. Also added `sumLabel` / `minLabel` / `maxLabel` / `averageLabel` support end-to-end so the total row labels can be customised/translated.

## 3.0.21

### Patch Changes

- 26c66f0: DateRangePicker mas limit is now 5 years from the current date

## 3.0.20

### Patch Changes

- ef68483: Add layout customization tokens to Canvas Overlay: actions-bar grid placement and self-alignment (4 tokens), templates grid with tablet/mobile responsive variants (7 tokens), and configurator layout + form card with responsive variants (9 tokens). 20 new tokens total under the `--em-canvas-overlay-*` namespace.

## 3.0.19

### Patch Changes

- 5cc3fa0: Update the tooltip placement

## 3.0.18

### Patch Changes

- b7508fb: Adds click handling to heatmap component
- 12e2d01: Add new tokens related to canvas

## 3.0.17

### Patch Changes

- 52e3a71: add --em-core-motion-\* primitive tokens (durations + easings)

## 3.0.16

### Patch Changes

- f63de5b: Update css variable names for the switch

## 3.0.15

### Patch Changes

- 723aec1: Change switch text font size

## 3.0.14

### Patch Changes

- 3fd5bd3: Release
- 31f68fe: Update monospace font on styles

## 3.0.13

### Patch Changes

- e33bb77: Fix KpiChart to not show NaN when value is null

## 3.0.12

### Patch Changes

- b7cb546: Add BubbleChart component

## 3.0.11

### Patch Changes

- 12e0534: Improve interactivity

## 3.0.10

### Patch Changes

- ea0880e: Add missing export for KpiTrend

## 3.0.9

### Patch Changes

- 10a83d0: Add Scatter plot chart

## 3.0.8

### Patch Changes

- b67bdc6: Extract KpiTrend as separate component

## 3.0.7

### Patch Changes

- 64da740: Update css variables

## 3.0.6

### Patch Changes

- ffbe154: ChartTabs

## 3.0.5

### Patch Changes

- 2ae2f89: Improve ticks on chartjs cartesian charts

## 3.0.4

### Patch Changes

- 18a6fc2: Remove input filled css variable

## 3.0.3

### Patch Changes

- 97cec76: Update the kpi chart to skip the reserved trend space if there is none

## 3.0.2

### Patch Changes

- b2960aa: Add Kpi label color

## 3.0.1

### Patch Changes

- 708aa85: Improve MultiSelectField display value

## 3.0.0

### Major Changes

- 8575852: Improve selectfields + Introduce customTriggers

## 2.0.46

### Patch Changes

- 56d9c51: KPI display null formats value if it is number

## 2.0.45

### Patch Changes

- c37bd9b: Add `disableApplyButton` to externally disable Apply and `onPendingChange` to observe pending selection changes

## 2.0.44

### Patch Changes

- 029e4f2: Markdown component

## 2.0.43

### Patch Changes

- 88cb78b: Fix horizontal bar chart datalabel

## 2.0.42

### Patch Changes

- 5a8c412: Fix BarChart label alignments

## 2.0.41

### Patch Changes

- d2d8f48: Introduce new Divider component

## 2.0.40

### Patch Changes

- f735a47: PivotTable with option of sub rows
- 1246393: Update buttons with the new design system

## 2.0.39

### Patch Changes

- 523977f: Fix KPI font change on value change

## 2.0.38

### Patch Changes

- 2438001: KPI charts display null as

## 2.0.37

### Patch Changes

- 23077c8: Update ActionIcon styles

## 2.0.36

### Patch Changes

- 2318678: Adjust Card tooltip position

## 2.0.35

### Patch Changes

- f9003ad: Update Card tooltip display

## 2.0.34

### Patch Changes

- 1196857: Update Card tooltip display

## 2.0.33

### Patch Changes

- a6897ef: Introduce card tooltip

## 2.0.32

### Patch Changes

- 29a43d4: Add ghost variant to the SingleSelectField

## 2.0.31

### Patch Changes

- abd6df7: Include global.css in the package

## 2.0.30

### Patch Changes

- 2a75fc3: Allow numbers and booleans on SingleSelectField and MultiSelectField

## 2.0.29

### Patch Changes

- 2cfb816: Add avoidCollisions to the components to use the Dropdown + the Dropdown

## 2.0.28

### Patch Changes

- 002b753: Update ChartHeader Styles

## 2.0.27

### Patch Changes

- 0a252ef: Update Design System Semantics

## 2.0.26

### Patch Changes

- 9953064: Cleanup CardHeader

## 2.0.25

### Patch Changes

- da8e49d: Introduce table cell styles

## 2.0.24

### Patch Changes

- ac14faa: Fix calendar arrow styles
- 10899ed: Fix publish pipeline

## 2.0.23

### Patch Changes

- f2593e9: Update package-lock

## 2.0.22

### Patch Changes

- cecb93e: TableScrollable

## 2.0.21

### Patch Changes

- 2b6aad9: Fix max width of the TableBody

## 2.0.20

### Patch Changes

- 8280488: Improve table.module.css styles

## 2.0.19

### Patch Changes

- e0adda8: HeatMap accepts with single color

## 2.0.18

### Patch Changes

- 95b8c28: Add DateRange animations and year select

## 2.0.17

### Patch Changes

- fe62ebf: Improve DateRangePickerField display value

## 2.0.16

### Patch Changes

- 739f086: Implement DateRangePicker css variables

## 2.0.15

### Patch Changes

- 3a37423: New DateRangePicker components

## 2.0.14

### Patch Changes

- 90a6196: Improve getChartjsAxisOptionsScalesGridColor

## 2.0.13

### Patch Changes

- cce4ac7: Improve KPI resize behavior

## 2.0.12

### Patch Changes

- 4857fe4: Improve KPI chart resize

## 2.0.11

### Patch Changes

- 40af84d: DonutChart hides if not enough available space + New resize hook

## 2.0.10

### Patch Changes

- f5b3a92: New release process

## 2.0.9

### Patch Changes

- 97e4674: New release process

## 2.0.8

### Patch Changes

- aeeeca4: New release process

## 2.0.7

### Patch Changes

- 6a58676: Test new release process

## 2.0.6

### Patch Changes

- cddd509: Test new release process

## 2.0.5

### Patch Changes

- 6d88a83: Fix DonutChart center alignment

## 2.0.4

### Patch Changes

- 298778f: README.md

## 2.0.3

### Patch Changes

- ee784f7: Fix PivotTable zero rows and column values

## 2.0.2

### Patch Changes

- 346fe87: Table charts default cell alignment to the center

## 2.0.1

### Patch Changes

- bdadf20: Fix release cmd

## 2.0.0

### Major Changes

- d85c185: Separate Remarkable UI from Remarkable PRO
